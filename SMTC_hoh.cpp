 ...
 *   
 * Description:
 *   hoh implementation of a TC graph
 * Copyright (c) 2018.
 ...
 public:
    ...
    /*      //validating TC, pred and curr are not marked and pred->next = curr
        ...
        void locateV(vlist_t ** pred, vlist_t ** curr, int key){
               ...
               pthread_mutex_lock(&(*curr)->lock);
                while((*curr)->vnext.load(memory_order_seq_cst) != NULL && (*curr)->val < key){
                    pthread_mutex_unlock(&(*pred)->lock);    
                    (*pred) = (*curr);
        ...
        void locateTC(tclist_t* TChead, tclist_t ** pred, tclist_t ** curr, int key){
               ...
               pthread_mutex_lock(&(*curr)->lock);
                while((*curr)->tcnext.load(memory_order_seq_cst) != NULL && (*curr)->val < key){
                    pthread_mutex_unlock(&(*pred)->lock);    
                    (*pred) = (*curr);
        ...
        void DFSFW(elist_t * slHead){
             ...
             //cin>>z;
             for(elist_t* i = slHead->enext.load(memory_order_seq_cst)->enext.load(memory_order_seq_cst) ; i->enext.load(memory_order_seq_cst)  != NULL; i = i->enext.load(memory_order_seq_cst) ){
             //cout<<"i->val:"<<i->val<<endl;
                     ...
                     if(flag == true){
                          ...
                          DFSFW(curr->enext.load(memory_order_seq_cst) );  
                         } 
                     }
                   }  
            ...
            // cout<<"A-i->val:"<<i->val<<endl;
                     ...
                     if(flag1 == true){
                         // cout<<"A-Bcurr & i:"<<curr->val<<" "<<i->val<<endl;        
                          addVTCInU(curr, v);      
                          DFSBW(curr->enext.load(memory_order_seq_cst) , v);  
                          }
                     }
                   }  
       ...
       bool updateTCAfterAddE(vlist_t *pred1, vlist_t *curr1, vlist_t *pred2, vlist_t * curr2){
           ...
           while(c2->tcnext.load(memory_order_seq_cst)  != NULL) {
             AddTC(curr1->tcnext.load(memory_order_seq_cst) , c2->val);
           ...
           //cout<<"A-ite1->val:"<<ite1->val<<endl;
              if(ite1->val < 0){
                ...
                if(flag1 == true){
                    ...
                    DFSBW(n1->enext.load(memory_order_seq_cst) , curr2); // call the DFS on the backward vertices 
                  }
               }     
      ...
      public:  
        ...
        bool AddVertexSMTC(int key){
         ...
         bool RemoveVertexSMTC(int key){
                vlist_t* pred, *curr;
                return lt.RemoveV(key, &pred, &curr);
        ...
        bool AddEdgeSMTC(int key1, int key2){
                ...
                if(flag3 == false)
                     return false;
                bool flag =  lt.AddE(curr1->enext.load(memory_order_seq_cst) , key2) && lt.AddE(curr2->enext.load(memory_order_seq_cst) , (-1)*key1);  
                if(flag == true){
                 bool flag4 =  lt.AddTC(curr1->tcnext.load(memory_order_seq_cst) , key2);
                 if(flag4 == true)  
